    ...
    return (
        <>
            ...
            <div className="flex-1 flex flex-col max-w-[768px] w-full mx-auto">
                ...
                <h1 className="text-[44px] font-semibold w-full px-4 py-4 rounded outline-none">{post.title}</h1>
                <div className="flex mx-4 gap-3 items-center my-2">
                    <UserAvatar user={post.user} className="w-11 h-11" />
                    <div>
                        <p>{post.user.name}</p>
                        <p className="text-gray-500">{post.createdAt.toLocaleDateString("ko")}</p>
                    </div>
                </div>
                <hr className="m-4" />
